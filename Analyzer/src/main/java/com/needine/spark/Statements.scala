...
object Statements {
  ...
  def savePacket(time: String, origen: Long, destiny: Long, bytes: Double): String = s"""
  ...
  def createKeySpace(session: Session) = {
    ...
    session.execute(
      """create table if not exists network_monitor.packet_by_origen_destiny (time text, origen bigint, destiny bigint, bytes double, primary key((origen, destiny), time))""")
